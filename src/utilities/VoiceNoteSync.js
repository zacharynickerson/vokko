...
class VoiceNoteSync {
  ...
  async getCloudVoiceNotes() {
    const snapshot = await get(dbRef(db, `users/${auth.currentUser.uid}/voiceNotes`));
  ...
  async uploadVoiceNote(note) {
    ...
    const audioRef = storageRef(storage, `users/${auth.currentUser.uid}/voiceNotes/${note.voiceNoteId}`);
    ...
    const noteRef = dbRef(db, `users/${auth.currentUser.uid}/voiceNotes/${note.voiceNoteId}`);
    await set(noteRef, { ...note, cloudUri, uri: cloudUri });
